...
class QTable:
	...
	def choose_action(self, state):
		self.check_state_exist(state) # Eger boyle bir durum daha once yoksa ekler 

		action=0
		if np.random.random() < self.epsilon:# Epsilon degerinin asagısındaysa  random bir aksiyon secer
...
# Start
QT = QTable() # ILk olarak nesne olusturuldu
...
for episode in range(1, 500):
	...
	while True:
		...
		QT.learn(state, state2, reward, action) # Q learning algoritması uygulanıyor
	...
	if G > max_r:  #Eger odulumuz  max rewarddan fazlaysa islemi yap
		max_r = G
	# if episode % 10 == 10:
