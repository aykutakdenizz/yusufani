...
class Brain():
	...
	def choose_action(self, state):
		state = state[np.newaxis, :]  # Yeni bir durum icin yeni bir satır aciliyor
		# action=0
		if np.random.random() < self.epsilon: # Eger belirlenen epsilon degerinden kucukse herhangi bir rastgele action atanıyor
			action = env.action_space.sample()
		else: # Eger degilse ???  anladıgım kadarıyla ilgili statedeki en yüksek ihtimalli aksiyon
			action_val = self.sess.run(self.predicted_eval, feed_dict={self.ntwrk_state: state})
			action = np.argmax(action_val) # En yüksek ihtimalli aksiyonu sec 
		return action
...
brain = Brain()
#Her 200 adımda bir yapacagı aksiyona karar veriyor. Ya sağa gidebilir ya da sola.Bir süre sonra çok daha fazla doğru karar veriyor
#173. satıra bir breakpoint koyup görebilirsin.
step = 0
for episode in range(1, 1000):

	state = env.reset() # Baslangıc durumundaki kosullatır state e atar
	print("Episode: ", episode)
	while True:
		env.render() # Oyunu görsel olarak baslatan komut
		...
		# Store transition in replay memory Gerekli bilgileri network e attı
